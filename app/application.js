	...
	*/
	...
	window.ai_v1_findNextActions = (playerId, difficulty) => new Promise(function(resolve, reject) {
        ...
        request.done(function(res){
            ...
            const actions = [];
            for (let actionData of Array.from(actionsData)) {
...
App._startGame = function() {
	...
	for (let cardIndex of Array.from(gameSession.getMyPlayer().getDeck().getHand())) {
...
App._startLoadingGameOverData = function(){
	...
	.spread(function(userGameModel,challengeModel){
		...
		const allPromises = [];
		if (rewardIds != null) {
			for (let rewardId of Array.from(rewardIds)) {
...
`;
	...
	Object.keys(browserlist).forEach(function(browser) {
