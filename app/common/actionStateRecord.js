...
class ActionStateRecord {
  ...
  startListeningToEvents(eventBus) {
    ...
    if (!this._listeningToEvents && (eventBus != null)) {
      ...
      if (eventTypes.length > 0) {
        return Array.from(eventTypes).map((eventType) => this._eventBus.on(eventType, this.onStateRecordingActionEvent, this));
      }
    }
  }
  ...
  recordStateEvenIfNotChanged(actionIndex) {
    ...
    const eventTypes = Object.keys(this._propertyNamesToRecordByEventType);
    return Array.from(eventTypes).map((eventType) => this._recordProperties(eventType, actionIndex, ignoreChanged));
  }
  ...
  recordStateAtLastActionRecorded() {
    ...
    const eventTypes = Object.keys(this._propertyNamesToRecordByEventType);
    return Array.from(eventTypes).map((eventType) => this._recordProperties(eventType, actionIndex));
  }
