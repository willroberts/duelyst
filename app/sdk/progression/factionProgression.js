 ...
 */
let card; let cardId; let cardsUpToLevel; let factionId; let i; let nonPrismaticCardsUpToLevel; let
  reward;
let asc1; let
  end1;
...
class FactionProgression {
  ...
  static levelForXP(xp_amount) {
    ...
    for (const level in this.levelXPTable) {
      let xp_required = this.levelXPTable[level];
...
for (factionId of Array.from(factionIds)) {
  ...
  const cardsUpToLevelForFaction = (FactionProgression._unlockedCardsUpToLevelByFaction[factionId] = {});
  const nonPrismaticCardsUpToLevelForFaction = (FactionProgression._unlockedNonPrismaticCardsUpToLevelByFaction[factionId] = {});
  cardsUpToLevel = [];
  nonPrismaticCardsUpToLevel = [];

  for (i = 0, end = FactionProgression.maxLevel, asc = end >= 0; asc ? i <= end : i >= end; asc ? i++ : i--) {
...
cardsUpToLevel = [];
nonPrismaticCardsUpToLevel = [];
for (i = 0, end1 = FactionProgression.maxLevel, asc1 = end1 >= 0; asc1 ? i <= end1 : i >= end1; asc1 ? i++ : i--) {
  for (factionId of Array.from(factionIds)) {
    factionId = parseInt(factionId);
    reward = FactionProgression.rewardDataForLevel(factionId, i);
