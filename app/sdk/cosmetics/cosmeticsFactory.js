...
class CosmeticsFactory {
  ...
  static cosmeticProductDataForType(cosmeticType, cosmeticSubType) {
    ...
    return _.map(cosmeticDatas, (cosmeticData) => this._mapCosmeticDataToProductData(cosmeticData));
  }
  ...
  static cosmeticResourcesForIdentifier(identifier) {
    ...
    if (cosmeticData != null) {
      ...
      if (cosmeticData.animResource != null) {
        for (const animResourceKey of Array.from(Object.keys(cosmeticData.animResource))) {
          const animResourceName = cosmeticData.animResource[animResourceKey];
          if (_.isString(animResourceName)) {
            const rsx = RSX[animResourceName];
...
for (cosmeticTypeKey in CosmeticsLookup) {
  const cosmeticsOfType = CosmeticsLookup[cosmeticTypeKey];
  if ((cosmeticsOfType == null) || !_.isObject(cosmeticsOfType)) {
    continue;
  }
  const cosmeticTypeId = CosmeticsTypeLookup[cosmeticTypeKey];
  if ((cosmeticTypeId == null)) {
    continue;
  }
  for (const cosmeticKey in cosmeticsOfType) {
...
for (const cosmeticChestTypeKey of Array.from(cosmeticChestTypes)) {
...
for (const cosmeticIdKey in cosmeticIdKeys) {
  ...
  const subType = cosmeticData.subTypeId;
...
for (const cosForDesc of Array.from(allCosForDesc)) {
...
for (const cosForOrder of Array.from(allCosForRewardOrder)) {
