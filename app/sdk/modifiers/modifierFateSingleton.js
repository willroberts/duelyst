...
class ModifierFateSingleton extends ModifierFate {
  ...
  onActivate() {
    if (this.getGameSession().getIsRunningAsAuthoritative()) {
      ...
      } else if (!general.hasActiveModifierClass(ModifierQuestStatusNeutral)) {
  ...
  checkDeckForDuplicates() {
    ...
    for (const card of Array.from(deckCards)) {
      for (const checkedId of Array.from(checkedCardIds)) {
  ...
  removeQuestStatusModifier() {
    const general = this.getGameSession().getGeneralForPlayerId(this.getCard().getOwnerId());
    if (general.hasActiveModifierClass(ModifierQuestStatusNeutral)) {
      return Array.from(general.getModifiersByClass(ModifierQuestStatusNeutral)).map((mod) => this.getGameSession().removeModifier(mod));
    }
  }
