...
class Player extends SDKObject {
  ...
  onEvent(event) {
    ...
    return Array.from(cards).map((card) => card.onEvent(event));
  }
  ...
  getPlayerModifiersByType(type) {
    const modifiers = [];
    for (const m of Array.from(this.getPlayerModifiers())) {
  ...
  propertiesForActionStateRecord() {
    ...
    return {
      numCards: () => this.getDeck().getNumCardsInDrawPile(),
      numCardsInHand: () => this.getDeck().getNumCardsInHand(),
      remainingMana: () => this.getRemainingMana(),
      maximumMana: () => this.getMaximumMana(),
    };
  }
