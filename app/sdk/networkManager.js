			...
			_NetworkManager = class _NetworkManager {
				...
				connect(gameId,playerId,gameServerAddress,spectatorId=null,spectateToken=null) {
					...
					if ((gameServerAddress == null)) {
						...
						this.socketManager = new io.Manager(`${window.location.hostname}:8000`, { auth: {token: `Bearer ${Storage.get('token')}`}, timeout: 20000, reconnection: false, reconnectionDelay: 500, reconnectionDelayMax: 5000, reconnectionAttempts: 1 });
					} else {
						this.socketManager = new io.Manager(`wss://${gameServerAddress}`, { auth: {token: `Bearer ${Storage.get('token')}`}, timeout: 20000, reconnection: true, reconnectionDelay: 500, reconnectionDelayMax: 5000, reconnectionAttempts: 20 });
					}
