  ...
  * @param {moment} systemTime
  ...
  static _buildCachesIfNeeded(systemTime) {
    ...
    if ((GameSession._cardsCachedAt == null) || (MOMENT_NOW_UTC.month() !== GameSession._cardsCachedAt.month())) {
      ...
      const factionsHash = {};
      for (const faction of Array.from(FactionFactory.getAllFactions())) {
      ...
      const cacheRecursive = function (cacheInto, cacheByMasterKey, cardsToCache, cacheByRemaining) {
        ...
        return Array.from(cacheByRemaining).map((cacheByData) => createLazySubCache(cache, cacheByData, cardsToCache, cacheByRemaining));
      };
      ...
      var createSubCache = function (cache, cacheByData, cardsToCache, cacheByRemaining) {
        ...
        // group cards
        for (const card of Array.from(cardsToCache)) {
        ...
        for (const subCacheByData of Array.from(cacheByRemaining)) {
...
class _GameSession extends SDKObject {
  ...
  _pushEventToSession(event, options) {
    ...
    for (const validator of Array.from(this.getValidators())) {
    ...
    for (const card of Array.from(this.getEventReceivingCards())) {
      card.onEvent(event);
  ...
  syncState() {
    ...
    for (const modifierIndex of Array.from(modifierIndices)) {
    ...
    return Array.from(this.players).map((player) => player.syncState());
  }
    ...
    if (!this.isOver() && !this.getIsBufferingEvents()) {
      ...
      for (player of Array.from(this.players)) {
        ...
        if ((general == null)) {
          ...
          const cardIndices = Object.keys(this.cardsByIndex);
          for (const index of Array.from(cardIndices)) {
  ...
  p_startTurn() {
    if (this.isActive()) {
      ...
      // update entities
      for (const entity of Array.from(this.board.getEntities(allowUntargetable = true))) {
  ...
  _executeActionForStep(actionToExecute, step) {
    ...
    for (const modifier of Array.from(changedByModifiers)) {
  ...
  getActionsOfClassInQueue(actionClass, targetPosition) {
    ...
    if (actionQueue != null) {
      for (const action of Array.from(actionQueue)) {
  ...
  getActions() {
    ...
    for (const index of Array.from(actionIndices)) {
  ...
  getSteps() {
    ...
    for (const index of Array.from(stepIndices)) {
  ...
  getCardsPlayed(playerId, cardClass) {
    ...
    for (const card of Array.from(this.getCards())) {
  ...
  deserializeSessionFromFirebase(sessionData) {
    ...
    if (sessionData.turns != null) {
      for (const turnData of Array.from(sessionData.turns)) {
    ...
    for (const cardIndex of Array.from(this.board.getCardIndices())) {
