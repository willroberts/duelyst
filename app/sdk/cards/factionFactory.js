...
class FactionFactory {
  ...
  static getAllFactions() {
    if ((this._allFactions == null)) {
      this._allFactions = [];
      for (const factionId of Array.from(_.chain(Factions).values().uniq().value())) {
  ...
  static starterDeckForFactionLevel(factionId, factionLevel) {
    ...
    if (factionLevel > 0) {
      ...
      for (const cardId of Array.from(unlockedCardIds)) {
        if (FactionFactory.unlockedCardIsValidForStarterDeck(cardId)) {
          for (let i = 0, end = CONFIG.MAX_DECK_DUPLICATES, asc = end >= 0; asc ? i < end : i > end; asc ? i++ : i--) {
  ...
  static factionForGeneralId(generalId) {
    ...
    for (const factionId of Array.from(factionIds)) {
