...
class SpellApplyModifiersToUnitsInHand extends Spell {
  ...
  onApplyToBoard(board, x, y, sourceAction) {
    ...
    return Array.from(this.getCardsAffected()).map((card) => Array.from(this.targetModifiersContextObjects).map((modifierContextObject) => this.getGameSession().applyModifierContextObject(modifierContextObject, card)));
  }
