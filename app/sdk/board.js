...
class Board extends SDKObject {
	...
	getPositions() {
		...
		for (let x = 0, end = this.columnCount, asc = 0 <= end; asc ? x < end : x > end; asc ? x++ : x--) {
			for (let y = 0, end1 = this.rowCount, asc1 = 0 <= end1; asc1 ? y < end1 : y > end1; asc1 ? y++ : y--) {
				positions.push({x,y});
			}
		}
		return positions;
	...
	getUnits(allowUntargetable, allowQueued) {
		...
		for (let unit of Array.from(this._private.units)) {
	...
	getTiles(allowUntargetable, allowQueued) {
		...
		for (let tile of Array.from(this._private.tiles)) {
	...
	getSpells(allowQueued) {
		...
		for (let spell of Array.from(this._private.spells)) {
	...
	getQueuedCardAtPosition(pos, type, allowUntargetable) {
		...
		if (allowUntargetable == null) { allowUntargetable = false; }
		for (let action of Array.from(this.getGameSession().getActionsOfClassInQueue(ApplyCardToBoardAction, pos))) {
	...
	getValidPositionsForModifierSourceType(card, modifierSourceType) {
		...
		const validMap = [];
		for (let entity of Array.from(this.getEntities())) {
			...
			if ((entity.getOwnerId() === playerId) && (modifierSource != null)) {
				...
				for (let validPosition of Array.from(sourceValidPositions)) {
	...
	getValidSpawnPositions(card) {
		const playerId = card.getOwnerId();
		const validMap = [];
		for (let entity of Array.from(this.getUnits(true))) { // can spawn around any allied units (including untargetable allies)
			if (entity.getOwnerId() === playerId) {
				...
				for (let validPosition of Array.from(validPositions)) {
					validMap[UtilsPosition.getMapIndexFromPosition(this.columnCount, validPosition.x, validPosition.y)] = validPosition;
				}
			}
		}
	...
	getCardsOutsideRadiusOfPosition(position, type, radius, allowUntargetable, allowQueued) {
		...
		if (position != null) {
			...
			if (radius <= 0) {
				...
				for (let card of Array.from(allCards)) {
	...
	getEnemyEntitiesAroundEntity(entity, type, radius, allowUntargetable, allowQueued) {
		...
		for (let nearby of Array.from(this.getEntitiesAroundEntity(entity, type, radius, allowUntargetable, allowQueued))) {
	...
	getEnemyEntitiesNotAroundEntity(entity, type, radius, allowUntargetable, allowQueued) {
		...
		for (let notNearby of Array.from(this.getEntitiesNotAroundEntity(entity, type, radius, allowUntargetable, allowQueued))) {
	...
	getFriendlyEntitiesInfrontOfEntity(entity, type, allowUntargetable, allowQueued) {
		...
		for (let otherEntity of Array.from(entitiesInfront)) {
	...
	getFriendlyEntitiesOnCardinalAxisFromEntityToPosition(entity, targetPosition, type, allowUntargetable, allowQueued) {
		...
		for (let otherEntity of Array.from(this.getEntitiesOnCardinalAxisFromEntityToPosition(entity, targetPosition, type, allowUntargetable, allowQueued))) {
	...
	getFriendlyEntitiesOnEntityStartingSide(entity, type, allowUntargetable, allowQueued) {
		...
		for (let otherEntity of Array.from(this.getEntitiesOnEntityStartingSide(entity, type, allowUntargetable, allowQueued))) {
