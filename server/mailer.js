...
class Email {
  ...
  sendMail(username, email, title, message, cb) {
    ...
    const html = template({
      title, base_url, username, message,
    });
    const opts = {
    ...
    return this.transporter.sendMail(opts, (error, info) => {
      if (error) {
        return cb(error);
      }
  ...
  sendEmailVerificationLink(username, email, token, cb) {
    ...
    const html = template({
      title: 'Verify Email', base_url, username, token,
    });
    const opts = {
      from: 'support@duelyst.com',
      to: email,
  ...
  sendSignup(username, email, token, cb) {
    ...
    const html = template({
      title: 'Welcome', base_url, username, token,
    });
    const opts = {
      from: 'support@duelyst.com',
      to: email,
  ...
  sendReceipt(username, email, receiptNumber, boosterCount, cb) {
    ...
    const html = template({
      title: 'Welcome', base_url, username, receipt_number: receiptNumber, booster_count: boosterCount,
    });
    const opts = {
      from: 'support@duelyst.com',
      to: email,
  ...
  resendSignup(username, email, token, cb) {
    ...
    const html = template({
      title: 'Confirm', base_url, username, token,
    });
    const opts = {
      from: 'support@duelyst.com',
      to: email,
  ...
  sendForgotPassword(username, email, token, cb) {
    ...
    const html = template({
      title: 'Reset', base_url, username, token,
    });
    const opts = {
      from: 'support@duelyst.com',
      to: email,
  ...
  sendErrorAlert(server, error, cb) {
    ...
    const html = template({
      title: 'Server Error', base_url, server: JSON.stringify(server), error,
    });
    const opts = {
      from: 'admin@duelyst.com',
  ...
  sendCrashAlert(server, error, cb) {
    ...
    const template = handlebars.compile(this.alert_template);
    const html = template({
      title: 'Server Crash', base_url, server: JSON.stringify(server), error, prettyErrorInfo,
    });
    const opts = {
      from: 'admin@duelyst.com',
