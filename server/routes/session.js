...
.then(function(data) {
    ...
    this.token = jwt.sign(payload, config.get('jwt.signingSecret'), options);
...
router.post("/session/", function(req, res, next) {
	...
	const email = result.value.email != null ? result.value.email.toLowerCase() : undefined;
	const username = result.value.username != null ? result.value.username.toLowerCase() : undefined;

	const getUserIdAsync = username ? UsersModule.userIdForUsername(username) : UsersModule.userIdForEmail(email);
...
router.post("/session/register", function(req, res, next) {
	...
	.then(function(inviteCodeData) { // captcha verification
		...
		return UsersModule.createNewUser(email,username,password,inviteCode,referralCode,campaignData,registrationSource);}).then(function(userId) {
		...
		return res.status(400).json(e);
	}).catch(Errors.AlreadyExistsError, function(e) {	// Specific error if the email/user already exists
		Logger.module("Session").error(`can not register because username ${(username != null ? username.blue : undefined)} or email already exists`.red);
...
*/
router.post("/session/email_available", function(req, res, next) {
