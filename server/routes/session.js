...
router.post('/session/', (req, res, next) => {
  ...
  return getUserIdAsync
    ...
    .catch(Errors.AccountDisabled, (e) => res.status(401).json({ message: e.message }))
    .catch(Errors.NotFoundError, (e) => res.status(401).json({ message: 'Invalid Email or Password' }))
    .catch(Errors.BadPasswordError, (e) => res.status(401).json({ message: 'Invalid Email or Password' }))
    .catch((e) => next(e));
});
...
router.post('/session/change_username', isSignedIn, (req, res, next) => {
  ...
  return UsersModule.changeUsername(user_id, new_username)
    .then(() => res.status(200).json({})).catch(Errors.AlreadyExistsError, (e) => {	// Specific error if the username already exists
