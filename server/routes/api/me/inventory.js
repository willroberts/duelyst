...
router.post('/card_collection/soft_wipe', function(req, res, next) {
	...
	.bind({})
	.then(userRow => hashHelpers.comparePassword(password, userRow.password)).then(function(match) {
		if (!match) {
		...
		]);}).spread(function(cardCountRows,cardLogRows,cardCollectionRow,spiritOrbOpenedRows){
		...
		return zlib.gzipAsync(JSON.stringify(backup));}).then(function(backupDataZipped){

		this.backupDataZipped = backupDataZipped;
		return InventoryModule.softWipeUserCardInventory(user_id);}).then(function() {
		...
		// respond and return
		res.status(200).json({});
		return Promise.resolve();}).catch(Errors.BadPasswordError, e => res.status(401).json({message:'invalid password'})).catch(Errors.BadRequestError, e => res.status(400).json({message:e.message})).catch(error => next(error));
});
