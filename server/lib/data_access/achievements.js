...
class AchievementsModule {
	...
	static updateAchievementsProgressWithGame(userId, gameId, gameData, isUnscored, isDraw) {
		...
		for (let achievementId in enabledAchievements) {
	...
	static _applyAchievementProgressMapToUser(userId,progressMap,gameId=null) {
                ...
                if (!row.completed_at && (row.progress >= enabledAchievements[achievementId].progressRequired)) {
                    ...
                    for (rewardType in object) {
                        ...
                        if (rewardType === 'spiritOrb') {
                            ...
                            const cardIds = [];
                            for (let c of Array.from(rewardValue)) {
                                if (parseInt(c)) {
                                    ...
                                    const cardsForFaction = SDK.GameSession.getCardCaches().getCardSet(cardSet).getFaction(factionId).getRarity(rarityId).getIsUnlockable(false).getIsCollectible(true).getIsPrismatic(false).getIsGeneral(false).getCards();
                                    const cardIdsToSample = c.sample || _.map(cardsForFaction,c => c.id);
                                    const cardId = _.sample(cardIdsToSample);
                                    for (let j = 1, end = c.count, asc = 1 <= end; asc ? j <= end : j >= end; asc ? j++ : j--) {
                                        cardIds.push(cardId);
                                    }
                                }
                            }
                            ...
                            const neutralCommonCards = SDK.GameSession.getCardCaches().getCardSet(SDK.CardSet.Core).getFaction(Faction.Neutral).getRarity(Rarity.Common).getIsUnlockable(false).getIsCollectible(true).getIsPrismatic(false).getCards();
                            ...
                            const neutralRareCards = SDK.GameSession.getCardCaches().getCardSet(SDK.CardSet.Core).getFaction(Faction.Neutral).getRarity(Rarity.Rare).getIsUnlockable(false).getIsCollectible(true).getIsPrismatic(false).getCards();
                            ...
                            const neutralEpicCards = SDK.GameSession.getCardCaches().getCardSet(SDK.CardSet.Core).getFaction(Faction.Neutral).getRarity(Rarity.Epic).getIsUnlockable(false).getIsCollectible(true).getIsPrismatic(false).getCards();
                            ...
                            const neutralLegendaryCards = SDK.GameSession.getCardCaches().getCardSet(SDK.CardSet.Core).getFaction(Faction.Neutral).getRarity(Rarity.Legendary).getIsUnlockable(false).getIsCollectible(true).getIsPrismatic(false).getCards();
                            ...
                            const factionLegendaryCards = _.filter(SDK.GameSession.getCardCaches().getCardSet(SDK.CardSet.Core).getRarity(Rarity.Legendary).getIsUnlockable(false).getIsCollectible(true).getIsPrismatic(false).getCards(), card => card.getFactionId() !== Faction.Neutral);
                            ...
                            for (let i = 1, end1 = rewardValue, asc1 = 1 <= end1; asc1 ? i <= end1 : i >= end1; asc1 ? i++ : i--) {
                    ...
                    if (rewardObject.cosmetics) {
                        for (let cosmeticId of Array.from(rewardObject.cosmetics)) {
                    ...
                    if (rewardObject.cosmetic_keys) {
                        for (let keyType of Array.from(rewardObject.cosmetic_keys)) {
                    ...
                    if (rewardType === 'newRandomCosmetics') {
                        for (let cosmeticParams of Array.from(rewardValue)) {
		...
		.then(function(fbRootRef){
			...
			for (let row of Array.from(this.updatedAchievements)) {
	...
	static giveMythronCard(txPromise, tx, userId, achievementId) {
		...
		.then(function(card_collection_data) {
			...
			for (let mythronCard of Array.from(mythronCards)) {
