			...
			if (existingUserId) {
				...
				return knex.transaction(function(tx){
					...
					.then(function(userRow){
						...
						return knex("users").where('id',userId).update(userUpdateParams).transacting(tx);}).then(() => SyncModule._bumpUserTransactionCounter(tx,userId))
					.then(tx.commit)
					.catch(tx.rollback);
				});
			}
...
//				return Promise.resolve(true)
			...
			for (let card of Array.from(deck)) {
				cardId = card.id;
			...
			let generalCount = 0;
			for (let gameSessionCard of Array.from(gameSessionCards)) {
...
//				xp_earned = if isWinner then SDK.FactionProgression.winXP else SDK.FactionProgression.lossXP
            ...
            if ((!isUnscored && this.factionProgressionRow) && SDK.FactionProgression.hasLeveledUp(this.factionProgressionRow.xp,this.factionProgressionRow.xp_earned)) {
                ...
                if (rewardData != null) {
                    ...
                    if (rewardData.gold != null) {
                        ...
                        for (let emote_id of Array.from(rewardData.emotes)) {
        ...
        .then(function(rootRef){
            ...
            for (let key in this.updateUserGameParams) {
...
//						@.hasReachedDailyWinCountBonusLimit = true
				...
				if (!isUnscored) {
					...
					if (this.codexChapterIdsEarned && (this.codexChapterIdsEarned.length !== 0)) {
						...
						for (let codexChapterIdEarned of Array.from(this.codexChapterIdsEarned)) {
							// set up reward data
							rewardData = {
								id:generatePushId(),
								user_id:userId,
	...
	*/
	static updateUserBossProgressionWithGameOutcome(userId,opponentId,isWinner,gameId,gameType,isUnscored,isDraw,gameSessionData,systemTime) {
        ...
        .then(function(userRow) {
            ...
            for (let eventId in bossEventsData) {
...
//				allPromises.push(InventoryModule.giveUserGold(txPromise,tx,userId,rewardData.gold,rewardData.reward_type))
	...
	static iterateNewPlayerCoreProgression(userId) {
		...
		.then(function(moduleProgression){
			...
			.spread(function(quests,questsComplete){
				...
				var nextStage = null;
				for (let s of Array.from(NewPlayerProgressionStageEnum.enums)) {
	...
	static anonymizeUser(userId){
		...
		.then(function(userRow) {
            ...
            Logger.module("UsersModule").error(`anonymizeUser() partial failure -> ${e.message}`.red));}).then(() => UsersModule.suspendUser(userId, "User requested account deletion."));
	}
}
