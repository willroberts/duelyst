	...
	*/
	...
	static updateUserCreditCardToken(userId,cardToken,cardLastFourDigits){
		...
		return knex.transaction(function(tx){
			...
			.then(function(userRow){
				...
				}).transacting(tx);}).then(() => DuelystFirebase.connect().getRootRef())
			.then(rootRef => FirebasePromises.set(rootRef.child("user-inventory").child(userId).child("wallet").child('card_last_four_digits'),cardLastFourDigits)).then(tx.commit)
			.catch(tx.rollback);
			}).bind(thisObj)
		.then(function(){
	...
	static deleteUserCreditCardToken(userId){
		...
		return knex.transaction(function(tx){
			...
			.then(rootRef => FirebasePromises.remove(rootRef.child("user-inventory").child(userId).child("wallet").child('card_last_four_digits'))).then(tx.commit)
			.catch(tx.rollback);
			}).bind(thisObj)
		.then(() => true);
	}
...
//			if email then mail.sendReceiptAsync(username,email,@.charge.id,productData.qty)
	...
	static purchaseProductOnSteam(opts){
		...
		var trxPromise = knex.transaction(function(tx){
			...
			.then(function(userRow){
				...
				return ShopModule._addChargeToUser(trxPromise, tx, this.userRow, userId, sku, amount, 'usd', opts.orderId, opts, "steam", NOW_UTC_MOMENT);}).then(() => ShopModule._awardProductDataContents(trxPromise, tx, userId, opts.orderId, productData, NOW_UTC_MOMENT)).then(function(value){
				return this.to_return = value;}).then(() => SyncModule._bumpUserTransactionCounter(tx,userId))
			.then(tx.commit)
			.catch(tx.rollback);
			}).bind(this_obj)
		.then(function() {
	...
	static _awardProductDataContents(txPromise,tx,userId,chargeId,productData,systemTime){
		...
		if (productData.type === "starter_bundle") {
			for (let orbsData of Array.from(productData.spirit_orbs)) {
				var asc2, end2;
