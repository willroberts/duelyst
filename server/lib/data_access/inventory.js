	...
	static isAllowedToUseCards(txPromise, tx, userId, cardIds) {
		...
		.spread(function(factionProgression,cardCollectionRow){
			...
			for (let factionData of Array.from(SDK.FactionFactory.getAllEnabledFactions())) {
				...
				factionLevel = 0;
				for (let factionProgressionRow of Array.from(factionProgression)) {
			...
			.then(function(cosmeticRows) {
				for (let cosmeticId of Array.from(cosmeticIds)) {
					...
					if (cosmeticData.purchasable || cosmeticData.unlockable) {
						let found = false;
						// all users have all non-purchasable and non-unlockable cosmetics
						for (let row of Array.from(cosmeticRows)) {
				...
				return new Promise((resolve, reject) => {
					...
					try {
						...
						const randomUnownedCardFromCollection = function(ownedCardsData,potentialCardPool) {
							...
							for (let cardId of Array.from(shuffledCardPool)) {
	...
	static disenchantDuplicateCards(userId,systemTime) {
		...
		var txPromise = knex.transaction(function(tx){
			...
			.then(userRow => knex('user_card_collection').where('user_id',userId).first().transacting(tx).forUpdate()).then(function(collectionRow){
				...
				for (let cardId in collectionRow.cards) {
					const cardData = collectionRow.cards[cardId];
					if (cardData.count > 3) {
						if (cardsCache.getCardById(parseInt(cardId)) != null) {
							for (let i = 3, end = cardData.count, asc = 3 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
  ...
  // TODO: Remove this and replace uses with giveUserCosmeticId
	...
	static _refreshUserCardCollection(trxPromise,trx,userId,cardCountRows,updateFirebase) {
		...
		.then(function(collectionRow){
			...
			for (let cardRow of Array.from(cardCountRows)) {
				if (cardRow.count > 0) {
