  ...
  static wipeUserData(userId) {
    ...
    return DuelystFirebase.connect().getRootRef()
      ...
      .then((userRatingsRows) => // User needs to be removed from redis for each season they have a rating for
        Promise.map(userRatingsRows, (ratingRow) => {
      ...
      .then(() => Logger.module('SyncModule').timeEnd(`wipeUserData() -> ${userId.blue} wiped`));
  }
  // ###*
  ...
  static _syncUserFromSQLToFirebase(userId, shouldSyncBuddyList) {
    ...
    return DuelystFirebase.connect().getRootRef()
      ...
      .spread(function (cardRows, cardCollection, rankHistoryRows, chargeRows, gauntletRun, gauntletTicketRows, spiritOrbRows, decks, progression, factionProgressionRows, questRows, rewardRows, challengeRows, newPlayerModules, completedAchievements, buddyRows, gameCounterRows, factionGameCounterRows, userRibbonRows, userRankRatings, userCodexRows, userCosmeticChests, userCosmeticChestKeys, userCosmeticInventory, userBossesDefeated, userRiftRuns) {
        ...
        const fbOrbs = {};
        for (const orb of Array.from(spiritOrbRows)) {
          const orbId = orb.id;
          delete orb.user_id;
          delete orb.id;
        ...
        const fbTickets = {};
        for (const ticket of Array.from(gauntletTicketRows)) {
        ...
        for (const factionProgression of Array.from(factionProgressionRows)) {
        ...
        // sync ribbons
        const fbRibbonData = {};
        for (const ribbon of Array.from(userRibbonRows)) {
        ...
        if (userRankRatings) {
          for (const seasonRankRating of Array.from(userRankRatings)) {
        ...
        for (const userRiftRun of Array.from(userRiftRuns)) {
        ...
        return knex.transaction((trx) => {
          ...
          return trx.insert(userData).into('users')
            ...
            .then(function () { // faction progression
              ...
              for (const factionId in this.factionProgression) {
            ...
            .then(function () { // cards collection
              ...
              let card; let cardId; let cards; let
                reward;
              const inserts = [];
              ...
              if (this.achievements != null ? this.achievements.completed : undefined) {
                for (const achievementId in this.achievements.completed) {
              ...
              // booster cards
              for (const packId in (this.inventory != null ? this.inventory['used-booster-packs'] : undefined)) {
              ...
              for (const log of Array.from(cardLog)) {
            ...
            .then(function () { // challenges
              ...
              // daily quest rewards
              for (const challengeType in this.challengeProgression) {
            ...
            .then(function () { // new player progression
              ...
              // daily quest rewards
              for (const moduleName in this.newPlayerProgression) {
