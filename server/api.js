...
// TODO : Put in module
const makeDirectory = cb => mkdirp((__dirname + '/../public/' + config.get('env')), function(err) {
...
.on('error', err => cb(err)).on('finish', () => cb(null));
...
const setupDevelopment = () => server.listen(config.get('port'), function() {
...
const setupProduction = () => makeDirectory(function(err) {
    if (err != null) {
        ...
        return getIndexHtml(config.get('s3_url'), function(err) {
            if (err != null) {
                Logger.module("SERVER").log("getIndexHtml() failed: " + err);
                return process.exit(1);
            } else {
                return getRegisterHtml(config.get('s3_url'), function(err) {
                    if (err != null) {
                        Logger.module("SERVER").log("getIndexHtml() failed: " + err);
                        process.exit(1);
                    }
