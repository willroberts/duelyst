...
program
  ...
  .action((filename) => Promise.resolve()
    ...
    .spread((data, answers) => encryptor.encryptDataToFileAsync(data, `${__dirname}/config/${filename}.secret`, answers.passkey))
    .then(() => {
...
program
  ...
  .action((filename) => Promise.resolve()
    ...
    .then((answers) => encryptor.decryptFileToDataAsync(`${__dirname}/config/${filename}.secret`, answers.passkey))
    .then(function (data) {
      console.log(data);
    ...
    .then(function (answers) {
      ...
      return encryptor.encryptDataToFileAsync(this.data, `${__dirname}/config/${filename}.secret`, answers.passkey);
    })
    .then(() => {
      console.log(('ALL DONE'.green));
      return process.exit(0);
