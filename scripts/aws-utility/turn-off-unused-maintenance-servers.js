...
.spread(function(aiInstances,gameInstances){ // after getting instances, load metric data from CloudWatch for CPU STEAL TIME
	...
	return this.gameInstances =  _.filter(gameInstances, i => i["Status"] === "online");}).then(function(){ // get HEALTH (player count) data for each server
	...
	return Promise.map(this.gameInstances,(instance,index)=> {
		...
		.then(function(response){
			...
			return bar.tick();}).catch(error => console.error(`${instance.Hostname} failed to load health: ${error.message}`));
	},
	{concurrency:25});}).then(function(){ // load CONSUL maintenance info data for each instance
	...
	return Promise.map(this.gameInstances,(instance,index)=> {
		...
		.spread((res, body) => JSON.parse(body)).then(function(response){
	...
	{concurrency:25});}).then(function(results){ // when all data is loaded, retire any instances that have HIGH steal time
...
console.log("ALL DONE")).catch(DidNotConfirmError, e => console.log("ABORT"));
