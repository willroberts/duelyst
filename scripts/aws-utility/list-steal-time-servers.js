...
.spread(function(aiInstances,gameInstances){ // after getting instances, load metric data from CloudWatch for CPU STEAL TIME
	...
	{concurrency:25});}).then(function(){ // get HEALTH (player count) data for each server
	...
	for (let instance of Array.from(this.retiredInstances)) {
	...
	return Promise.all(allPromises);}).then(function(results){ // start each new substitute instance
...
console.log("ALL DONE")).catch(DidNotConfirmError, e => console.log("ABORTED"));
