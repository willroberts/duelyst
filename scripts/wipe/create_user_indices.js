...
const getAllUsernames = (cb) => fbAuthRef.child('user').once('value', (snapshot) => {
  const data = snapshot.val();
  const usernames = {};
  console.log(data);
  for (const user in data) {
...
const createUsernameIndex = (username, id) => fbRef.child('username-index').child(username).set(id, (error) => {
  if (error) {
