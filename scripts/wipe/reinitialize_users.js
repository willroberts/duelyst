...
const fbRef_saveBuddyLists = (cb) => fbRef.child('users').once('value', (snapshot) => {
...
const fbRef_setupProfile = (id, email, username) => new Promise((resolve, reject) => {
  const profile_data = {
    id,
    email,
    username,
  ...
  return userRef.setWithPriority(profile_data, email, (error) => {
    if (error) {
...
const fbRef_restoreBuddyLists = (id) => new Promise((resolve, reject) => {
  if (!buddyLists[id]) {
    return resolve();
  }

  const buddies = buddyLists[id];
  return fbRef.child('users').child(id).child('buddies').set(buddies, (error) => {
    if (error) {
...
const authRef_getAllUsers = (cb) => fbAuthRef.child('user').on('child_added', (snapshot) => {
  const data = snapshot.val();
...
fbRef_saveBuddyLists(() => fbRef_wipeRoot(() => authRef_getAllUsers((key, data) => {
  ...
  return fbRef_setupUser(key, data.email, data.username)
    .then(() => {
      ...
      return fbRef_restoreBuddyLists(key);
    }).catch((e) => {
