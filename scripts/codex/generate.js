...
Promise.resolve()
.bind({})
.then(() => require('./localization_index.coffee')).then(function(){
 ...
 this.cards = [
	...
	*/
	...
	return this.cardsJson = _.map(this.cards, function(c){
		const cardData = {
			id: c.id,
	...
	return Promise.map(this.cards, card=> {
...
var exportCardImage = function(cardIdentifier,showDetails,gif){
    ...
    ])).spread(function(animationData,cardSpriteSheet,cardBg,manaCrystal,atk,hp,shadow,rarityIcon){
...
const exportCardDescriptionImage = function(cardIdentifier){
	...
	.spread(function(rarityIcon,cardBg){
		...
		return new Promise((resolve, reject) => stream.on('end', () => resolve()));}).catch(function(e){
		console.error(`error exporting card ${card.name}`, e);
		throw e;
	});
};
