...
DuelystFirebase.connect(firebaseAuthUrl, firebaseAuthToken).getRootRef()
  ...
  .then(function (fbRootRef) {
    ...
    return new Promise((resolve, reject) => fbRootRef.child('users').limitToLast(1).on('child_added', (snapshot) => {
      last_user_id = snapshot.key();
      console.log((`LAST USER ID: ${last_user_id.blue}`));

      if (!has_started) {
        has_started = true;
        return fbRootRef.child('users').on('child_added', (snapshot) => {
          ...
          const userUpdatePromise = new Promise((resolveUpdate, rejectUpdate) => {
            console.log(`processing user ${user_id.blue}`.cyan);
            return fbAuthRef.child('user').child(user_id).once('value', (auth_user_snapshot) => {
              ...
              if (auth_user_snapshot != null ? auth_user_snapshot.val() : undefined) {
                ...
                return user_ref.update({ createdAt: created_at }, (error) => {
                  if (error) {
