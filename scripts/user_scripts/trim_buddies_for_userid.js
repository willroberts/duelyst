...
const trim_buddies_for_userid = function (userId) {
  ...
  return DuelystFirebase.connect().getRootRef()
    .then((fbRootRef) => {
      ...
      return get_conversation_partners_of_userid(userId);
      ...
      return new Promise((resolve, reject) => locFbRootRef.child('users').orderByKey().limitToLast(1).once('child_added', (snapshot) => {
    ...
    .then((finalUserKey) => // go through all users and remove this user from their buddy list
      new Promise((resolve, reject) => {
        ...
        return usersOn = locFbRootRef.child('users').orderByKey().on('child_added', (snapshot) => {
          usersProcessed++;
          ...
          if (!dryRun) {
            return locFbRootRef.child('users').child(snapshot.key()).child('buddies').child(userId)
              .remove((err) => {
    ...
    .then(() => {
      ...
      for (var conversationPartner of Array.from(locConversationPartners)) {
        if (!dryRun) {
          ...
          promises.push(new Promise((resolve, reject) => locFbRootRef.child('users').child(conversationPartner).child('buddies').child(userId)
            .set({ id: userId }, (err) => {
              if (err) {
          ...
          promises.push(new Promise((resolve, reject) => locFbRootRef.child('users').child(userId).child('buddies').child(conversationPartner)
            .set({ id: conversationPartner }, (err) => {
              if (err) {
                console.log(`trim_buddies_for_userid: Error adding user ${userId} to buddy list of user ${conversationPartner}`);
                return reject(`${userId} -> ${conversationPartner}`);
              }
...
if (process.argv[1].toString().indexOf('trim_buddies_for_userid.coffee') !== -1) {
  ...
  trim_buddies_for_userid(userId)
    .then((relationshipsAdded) => {
      ...
      console.log('Results: ');
      for (const result of Array.from(relationshipsAdded)) {
